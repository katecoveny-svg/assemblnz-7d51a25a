...
const ScoreBadge = ({ score }: { score: number }) => {
  ...
  const color = score >= 8 ? KINDLE_COLOR : score >= 5 ? "#00FF88" : "#EF4444";
  return (
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[180px] max-w-sm">
          ...
          <input type="text" placeholder="Search causes..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-8 pr-3 py-2 rounded-lg text-xs bg-background/50 border border-white/10 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1" style={{ focusRingColor: KINDLE_COLOR }} />
        </div>
