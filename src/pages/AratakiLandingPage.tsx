      ...
      <div style={{ minHeight: "100vh" }}>
        ...
        <main className="relative flex flex-col items-center px-6 pt-20 pb-28 text-center overflow-hidden">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
            className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(74,165,168,0.15)" }}>
          ...
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="text-lg md:text-xl max-w-2xl mb-10" style={{ color: "#6B7280" }}>
              ...
              <motion.div key={s.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                {s.route ? (
                  <Link to={s.route} className="block rounded-2xl p-6 h-full group transition-all hover:scale-[1.02]"
                    ...
                    <s.icon size={24} className="text-assembl-text/40 group-hover:text-white/70 transition-colors mb-4" />
                    <h3 className="text-base font-semibold text-white/90 mb-2 flex items-center gap-2">{s.title} <ChevronRight size={14} className="text-gray-300 group-hover:text-gray-500" /></h3>
                    <p className="text-sm text-gray-500">{s.desc}</p>
                  </Link>
                ) : (
                  <div className="rounded-2xl p-6 h-full" style={{ background: "rgba(255,255,255,0.65)", border: "1px solid rgba(74,165,168,0.15)" }}>
                    <s.icon size={24} className="text-assembl-text/40 mb-4" />
