    ...
    <div className="min-h-screen flex flex-col" style={{ background: "hsl(var(--background))" }}>
      ...
      <section className="relative z-10 pt-24 pb-16 px-4 sm:px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-3xl sm:text-5xl font-syne font-extrabold text-foreground mb-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={item.title} className="rounded-xl p-4 border border-border bg-card" style={{ backdropFilter: "blur(12px)" }}>
                  ...
                  <p className="text-xs font-syne font-bold text-foreground mb-1">{item.title}</p>
                  <p className="text-[10px] font-jakarta text-muted-foreground">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="relative z-10 py-16 border-t border-border">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl font-syne font-extrabold text-center text-foreground mb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {MARKET_STATS.map((s) => (
              <div key={s.label} className="text-center rounded-xl p-5 border border-border bg-card">
                <p className="text-2xl sm:text-3xl font-syne font-extrabold text-gradient-hero mb-1">{s.value}</p>
                <p className="text-[10px] font-jakarta text-muted-foreground">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {ROADMAP.map((item, i) => (
              ...
              >
                ...
                <div>
                  <p className="text-sm font-syne font-bold text-foreground mb-1">{item.title}</p>
                  <p className="text-xs font-jakarta text-muted-foreground">{item.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="relative z-10 py-16 border-t border-border">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <Link to="/#contact" className="inline-flex items-center gap-2 mt-4 text-sm font-syne font-bold text-primary hover:text-foreground transition-colors">
