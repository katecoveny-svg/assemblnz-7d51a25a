    ...
    <section className="py-16 border-t border-border">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-6">
          <h2 className="font-syne font-bold text-2xl sm:text-3xl text-foreground mb-2">Try NEXUS now</h2>
          <p className="text-sm text-muted-foreground font-jakarta">Ask NEXUS anything about customs, tariffs, or trade compliance. No signup required.</p>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border" style={{ background: `${NEXUS_COLOR}08` }}>
            ...
            <div>
              <p className="text-sm font-syne font-bold text-foreground">NEXUS</p>
          ...
          <div ref={scrollRef} className="h-[340px] overflow-y-auto p-4 space-y-3">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2 ml-10">
                  {STARTER_PROMPTS.map((p) => (
                    <button key={p} onClick={() => sendMessage(p)} className="text-xs font-jakarta px-3 py-1.5 rounded-full border transition-all hover:scale-105" style={{ borderColor: `${NEXUS_COLOR}30`, color: NEXUS_COLOR, background: `${NEXUS_COLOR}08` }}>
                      {p}
                    </button>
                  ))}
                </div>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
                ...
                <div className={`rounded-xl px-4 py-3 text-sm font-jakarta max-w-[85%] whitespace-pre-wrap ${m.role === "user" ? "rounded-tr-sm text-primary-foreground" : "rounded-tl-sm text-foreground bg-muted/50"}`} style={m.role === "user" ? { background: NEXUS_COLOR, color: "#fff" } : undefined}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="flex items-center gap-2 p-3 border-t border-border">
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about tariffs, duties, FTAs..." className="flex-1 bg-transparent text-sm font-jakarta text-foreground placeholder:text-muted-foreground outline-none px-3 py-2 rounded-lg border border-border" disabled={loading} />
          ...
          <div className="px-4 py-2 border-t border-border text-center">
            <Link to="/chat/customs" className="text-xs font-jakarta hover:underline" style={{ color: NEXUS_COLOR }}>Open full NEXUS experience →</Link>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden pt-24 pb-16 sm:pt-32 sm:pb-24">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
          ...
          <motion.div className="flex flex-wrap gap-3 justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
            <a href="#try-nexus" className="inline-flex items-center gap-2 px-8 py-4 rounded-xl text-base font-syne font-bold transition-all duration-300 hover:scale-105" style={{ background: NEXUS_COLOR, color: "#fff", boxShadow: `0 0 30px ${NEXUS_COLOR}30` }}>
              Try NEXUS Free <ArrowRight size={18} />
            </a>
            <button onClick={() => setShowVoice(true)} className="inline-flex items-center gap-2 px-6 py-4 rounded-xl text-base font-syne font-bold border transition-all duration-300 hover:scale-105" style={{ borderColor: `${NEXUS_COLOR}30`, color: NEXUS_COLOR, background: `${NEXUS_COLOR}08` }}>
      ...
      <section className="py-16 border-t border-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="font-syne font-bold text-2xl sm:text-3xl text-center mb-3 text-foreground">Built for NZ trade</h2>
          ...
          <div className="grid sm:grid-cols-3 gap-6 mb-10">
            {[
              ...
              <motion.div key={s.label} className="rounded-xl border border-border bg-card p-6 text-center" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                <p className="text-3xl sm:text-4xl font-syne font-black mb-2" style={{ color: NEXUS_COLOR }}>{s.stat}</p>
      ...
      <section className="py-16 border-t border-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="font-syne font-bold text-2xl sm:text-3xl text-center mb-3 text-foreground">Complete trade toolkit</h2>
          <p className="text-center text-muted-foreground text-sm mb-10 font-jakarta">From invoice processing to border compliance, NEXUS handles it all.</p>
      ...
      <section className="py-20 border-t border-border">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="font-syne font-black text-3xl sm:text-4xl text-foreground mb-4">Trade smarter, clear faster.</h2>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link to="/chat/customs" className="inline-flex items-center gap-2 px-8 py-4 rounded-xl text-base font-syne font-bold transition-all duration-300 hover:scale-105" style={{ background: NEXUS_COLOR, color: "#fff", boxShadow: `0 0 30px ${NEXUS_COLOR}30` }}>
