...
export default function DriverCompliance() {
  return (
    <div style={{ background: "#09090F", minHeight: "100vh", color: "#3D4250" }}>
      ...
      <main className="max-w-5xl mx-auto px-6 pt-16 pb-32">
        ...
        <p className="text-white/50 mb-10 max-w-xl">WoF/CoF expiry, RUC balance, licence class watch, and logbook prompts.</p>

        <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="grid grid-cols-7 text-xs font-semibold text-white/40 uppercase tracking-wider px-6 py-3 border-b border-white/5">
          ...
          {VEHICLES.map((v, i) => (
            <motion.div key={v.reg} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.08 }}
              ...
              <span className={`font-mono text-xs ${v.rucKm < 500 ? "text-red-400" : v.rucKm < 2000 ? "text-amber-400" : "text-white/50"}`}>{v.rucKm.toLocaleString()} km</span>
              <span className="text-white/60">{v.driver}</span>
              <span className="text-white/50 text-xs">{v.licence}</span>
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)" }}>
            ...
            <p className="text-xs text-white/50">PLQ 901 — RUC balance at 142km. Purchase additional RUC before tomorrow's run or assign to local duties only.</p>
          </div>
          <div className="p-4 rounded-xl" style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.15)" }}>
            ...
            <p className="text-xs text-white/50">MXA 412 — WoF expires 19 April (4 days). Book inspection this week to avoid compliance gap.</p>
          </div>
        </div>
      </main>
      <BrandFooter />
